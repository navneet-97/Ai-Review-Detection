...
class PSRDetector:
    ...
    def calculate_ttr(self, text):
        ...
        tokens = [token.text.lower() for token in doc if not token.is_punct and not token.is_space and token.text.strip()]
    ...
    def train(self, texts, labels):
        print("Extracting PSR and TTR features...")
        features = []
        ...
        print(f"Coefficient (TTR): {self.model.coef_[0][1]:.4f}")
        ...
        self.analyze_feature_distributions(X, labels)
    ...
    def predict(self, text):
        ...
        psr = self.calculate_psr(text)
        ttr = self.calculate_ttr(text)
        features_scaled = self.scaler.transform([[psr, ttr]])
    ...
    def analyze_feature_distributions(self, X, labels):
        """
        Analyze and visualize PSR and TTR distributions
        """
        ...
        human_ttr = ttr_values[labels == 0]
        ai_ttr = ttr_values[labels == 1]
        
        # Create visualization
        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        
        # PSR Histogram
        axes[0, 0].hist(human_psr, bins=30, alpha=0.7, label='Human', color='blue')
        axes[0, 0].hist(ai_psr, bins=30, alpha=0.7, label='AI', color='red')
        axes[0, 0].set_xlabel('PSR Value')
        axes[0, 0].set_ylabel('Frequency')
        axes[0, 0].set_title('PSR Distribution by Class')
        axes[0, 0].legend()
        
        # PSR Box plot
        psr_data = [human_psr, ai_psr]
        axes[0, 1].boxplot(psr_data, labels=['Human', 'AI'])
        axes[0, 1].set_ylabel('PSR Value')
        axes[0, 1].set_title('PSR Distribution Box Plot')
        
        # TTR Histogram
        axes[1, 0].hist(human_ttr, bins=30, alpha=0.7, label='Human', color='blue')
        axes[1, 0].hist(ai_ttr, bins=30, alpha=0.7, label='AI', color='red')
        axes[1, 0].set_xlabel('TTR Value')
        axes[1, 0].set_ylabel('Frequency')
        axes[1, 0].set_title('TTR Distribution by Class')
        axes[1, 0].legend()
        
        # TTR Box plot
        ttr_data = [human_ttr, ai_ttr]
        axes[1, 1].boxplot(ttr_data, labels=['Human', 'AI'])
        axes[1, 1].set_ylabel('TTR Value')
        axes[1, 1].set_title('TTR Distribution Box Plot')
        ...
        # Print statistics
        print(f"\nPSR Statistics:")
        print(f"Human Reviews PSR - Mean: {human_psr.mean():.4f}, Std: {human_psr.std():.4f}")
        print(f"AI Reviews PSR - Mean: {ai_psr.mean():.4f}, Std: {ai_psr.std():.4f}")
        
        print(f"\nTTR Statistics:")
        print(f"Human Reviews TTR - Mean: {human_ttr.mean():.4f}, Std: {human_ttr.std():.4f}")
        print(f"AI Reviews TTR - Mean: {ai_ttr.mean():.4f}, Std: {ai_ttr.std():.4f}")
        
        # Feature correlation
        correlation = np.corrcoef(psr_values, ttr_values)[0, 1]
        print(f"\nPSR-TTR Correlation: {correlation:.4f}")
...
def main(csv_file_path):
    """
    Main function to run the PSR and TTR analysis
    """
    # Load data
...
def test_review(detector, review_text):
    ...
    print(f"TTR Value: {result['ttr_value']:.4f}")
